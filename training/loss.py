...
class CMLoss:
    ...
    def __call__(self, net, net_target, net_teacher, images, labels=None, augment_pipe=None, stf=False, pfgmpp=False, ref_images=None):
        ...
        # Time step discretization. (recall that sigma=t for EDM)
        step_indices = torch.arange(num_steps, dtype=torch.float64, device=images.device)
        t_steps = (sigma_max ** (1 / rho) + step_indices / (num_steps - 1) * (sigma_min ** (1 / rho) - sigma_max ** (1 / rho))) ** rho
        t_steps = torch.cat([net_teacher.round_sigma(t_steps), torch.zeros_like(t_steps[:1])]) # t_N = 0
        
        # Sample index from discrete uniform dist 
        idx = torch.randint(0,num_steps-2, (1,images.size(0)),device=images.device)[0,:]
        sigma = torch.index_select(t_steps,0,idx)
        sigma_next = torch.index_select(t_steps,0,idx+1)
        ...
        if pfgmpp:
            ...
            sigma = sigma.reshape((len(sigma), 1, 1, 1))
            weight = (sigma ** 2 + self.sigma_data ** 2) / (sigma * self.sigma_data) ** 2 # just set to 1 
