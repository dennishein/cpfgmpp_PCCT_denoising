...
@persistence.persistent_class
class cDhariwalUNet(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution at input/output.
        in_channels,                        # Number of color channels at input.
        out_channels,                       # Number of color channels at output.
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            for idx in range(num_blocks + 1):
                ...
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=(res in attn_resolutions), **block_kwargs)
        self.out_norm = GroupNorm(num_channels=cout)
        self.out_conv = Conv2d(in_channels=cout, out_channels=in_channels//2, kernel=3, **init_zero)
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False,  **model_kwargs):
        ...
        #print(x.size())
        if x.size(1) > 1:
          D_x = c_skip * x[:,n_mat:,:,:] + c_out * F_x.to(torch.float32)
        else:
          D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x
...
#----------------------------------------------------------------------------
# Consistency model preconditioning (same as EDM for now)
@persistence.persistent_class
